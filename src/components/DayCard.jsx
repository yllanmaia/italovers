    ...
    >
      <header className="mb-4">
        <div className="flex items-center gap-2">
          <span className="rounded-full bg-elevated px-2.5 py-1 text-[0.6875rem] font-bold tracking-wide text-fg-dim uppercase tabular-nums">
            Dia {dayNumber} · {formatDateShort(day.date)}
          </span>
          {segmento?.parcial && segmento.periodo && (
            <span className="rounded-full bg-elevated px-2.5 py-1 text-[0.6875rem] font-bold tracking-wide text-fg-faint uppercase">
              {segmento.periodo}
            </span>
          )}
          {isToday && (
            <span className="rounded-full bg-accent px-2.5 py-1 text-[0.6875rem] font-bold tracking-wide text-white uppercase">
              Hoje
            </span>
          )}
        </div>
        <h2 className="mt-2 text-[1.0625rem] leading-snug font-bold text-fg">
          {day.title}
        </h2>
        ...
        {day.note_merge && blocks[0] === day.blocks[0] && (
          <p className="mt-2 rounded-xl bg-elevated px-3 py-2 text-[0.8125rem] leading-snug text-fg-dim">
            {day.note_merge}
          </p>
        )}
      </header>
